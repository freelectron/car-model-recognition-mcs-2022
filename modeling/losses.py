...
class ContrastiveLoss(nn.Module):
    ...
    def __init__(self, margin: float = 1.0, euclidean_distance: torch.nn = PAIRWISEDISTANCE_P2):
    ...
    def forward(self, output1, output2, label):
        ...
        squared_margin = torch.square(torch.maximum(self.margin - euclidean_distance, zero_vec))
        loss_contrastive = torch.mean(label * squared_distance + (1 - label) * squared_margin)
