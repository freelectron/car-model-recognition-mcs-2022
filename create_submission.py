...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for i, (image1, image2) in tqdm(
            enumerate(test_loader, 0), total=len(test_loader)
        ):
    ...
    all_outputs_df_name = (
        f"siamese_classification_ods_dataset_outputs_epoch_{epoch}.csv"
    )
    submit_pairs_df.to_csv(os.path.join(results_model_run_folder, all_outputs_df_name))
    ...
    submit_pairs_df[["id", "score"]].to_csv(
        os.path.join(
            results_model_run_folder,
            f"siamese_classification_submission_epoch_{epoch}.csv",
        ),
        index=False,
    )
