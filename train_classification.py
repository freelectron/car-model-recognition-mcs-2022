...
def train(
    ...
    for i, data in enumerate(tqdm(dataloader_instance)):
        ...
        sigmoid_output_vectors += sigmoid_output_difference_vector.squeeze().detach().cuda().tolist()
...
def validate(
    ...
    with torch.no_grad():
        ...
        for i, data_image_pair in enumerate(dataloader_instance, 0):
            ...
            sigmoid_outputs_difference_vector += sigmoid_output_difference_vector.detach().cuda().tolist()
    ...
    roc_auc_score_epoch = roc_auc_score(true_label_values, sigmoid_outputs_difference_vector)
...
if __name__ == "__main__":
    ...
    load_model_from_state_dictionary = os.path.join(load_model_folder, "model_0_epoch.pt")
